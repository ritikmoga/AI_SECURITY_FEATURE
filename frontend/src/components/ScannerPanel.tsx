...
export function ScannerPanel({ page, onReport }: { page: Exclude<DashboardPage, 'reports' | 'admin'>; onReport: (report: ScanReport) => void }) {
  ...
  return <section className="panel scanner-panel"><div className="section-title-row"><div><p className="eyebrow">{details.eyebrow}</p><h2>{details.title}</h2><p className="muted">{details.description}</p></div>{loading ? <span className="loading-pill">Analyzing…</span> : null}</div>{page === 'url' ? <UrlScanForm {...props} /> : null}{page === 'message' ? <MessageScanForm {...props} /> : null}{page === 'file' ? <FileScanForm {...props} /> : null}{page === 'qr' ? <QrScanForm {...props} /> : null}{page === 'lookup' ? <ReportLookupForm {...props} /> : null}{error ? <div className="alert error">{error}</div> : null}</section>;
}
